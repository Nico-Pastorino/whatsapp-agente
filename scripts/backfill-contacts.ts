...
async function run(): Promise<void> {
  ...
  const legacy = (convs as ConvRow[]).filter(
    (c) => c.phone_jid?.endsWith("@s.whatsapp.net")
  );
  const skipped = convs.length - legacy.length;

  console.log(`Total sin contact_id: ${convs.length}`);
